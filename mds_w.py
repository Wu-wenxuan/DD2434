...
def clean_data(data):
    ...
    newData = data[:, 1:-1]
    label = data[:,-1]
    newData_with_name = numpy.c_[data[:,0],newData]
...
#     :return:
#     '''
#     D=numpy.zeros((X.shape[0],X.shape[0]))
#     for i in range(X.shape[0]):
#         for j in range(Y.shape[0]):
#             #print(numpy.sqrt(numpy.sum(numpy.power((X[i]-Y[j]),2))))
#             D[i,j] = numpy.sqrt(numpy.sum(numpy.power((X[i]-Y[j]),2)))
#     #print(D.shape)
#     return D
...
def cal_B(D):
    ...
    S = numpy.linalg.eigvals(B)
    print(B)
    #C = numpy.linalg.cholesky(B)
    # print(B-B.T)
    # print("S",S)
